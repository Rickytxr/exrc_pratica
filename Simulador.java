...
public class Simulador
{
    ...
    private static final double ONCA_CREATION_PROBABILITY = 0.06;
    ...
    public Simulador(int depth, int width)
    {
        ...
        view.setColor(Onca.class, Color.BLACK);
        ...
        for(int row = 0; row < field.getDepth(); row++) {
            for(int col = 0; col < field.getWidth(); col++) {
                if(rand.nextDouble() <= FOX_CREATION_PROBABILITY) {
                    ...
                    animals.add(rabbit);
                } else if (rand.nextDouble() <= ONCA_CREATION_PROBABILITY) {
                    Localizacao location = new Localizacao(row,col);
                    Onca onca = new Onca(true,field,location);
                    animals.add(onca);
                }
